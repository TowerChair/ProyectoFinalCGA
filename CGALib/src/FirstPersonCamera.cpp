...
void FirstPersonCamera::moveFrontCamera(bool dir, float dt,bool maxSpeed =false) {
    // Ignorar la componente Y del vector front
    glm::vec3 frontMove = glm::normalize(glm::vec3(this->front.x, 0.0, this->front.z));
    if (dir){
		if(maxSpeed){
			this->position += (float)dt * (3*speed)* frontMove;
		}else			
			this->position += (float)dt * speed* frontMove;
	}
    else{
		if(maxSpeed){
						this->position -= (float)dt * speed* frontMove;
		...
		else        this->position -= (float)dt * speed * frontMove;
...
void FirstPersonCamera::moveRightCamera(bool dir, float dt) {
    // Ignorar la componente Y del vector right
    glm::vec3 rightMove = glm::normalize(glm::vec3(this->right.x,0.0, this->right.z));
    if (dir)
        this->position += (float)dt * speed * rightMove;
    else
        this->position -= (float)dt * speed * rightMove;
    updateCamera();
}
